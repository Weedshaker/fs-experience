...
export default class CoinCalcOner extends CoinCalcGram {
  constructor () {
    super()

    this.changeListenerPieces = event => {
      const input = event.composedPath()[0]
      if (input.getAttribute('id') === 'request-silver-coin-price-by-pieces') this.root.querySelector('#request-silver-coin-price-by-gram-in-chf').value = input.value * 5
    }
  }
  ...
  renderHTML () {
    ...
    this.root.querySelector('h2').textContent = '1-Fr. Stückzahl zu Fiat-CHF'
    ...
    div.innerHTML = /* HTML */`
      <div class="input">
        <input type="number" id="request-silver-coin-price-by-pieces" name="request-silver-coin-price-by-pieces" min="1" placeholder="Wieviel 1-Fr. Stücke?"><span>&nbsp;Stücke</span>
      </div>
      <div class=equal>=</div>
    `
