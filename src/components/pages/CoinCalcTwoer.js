...
export default class CoinCalcTwoer extends CoinCalcGram {
  constructor () {
    super()

    this.changeListenerPieces = event => {
      const input = event.composedPath()[0]
      if (input.getAttribute('id') === 'request-silver-coin-price-by-pieces') this.root.querySelector('#request-silver-coin-price-by-gram-in-chf').value = input.value * 10
    }
  }
  ...
  renderHTML () {
    ...
    div.innerHTML = /* HTML */`
      <div class="input">
        <input type="number" id="request-silver-coin-price-by-pieces" name="request-silver-coin-price-by-pieces" min="1" placeholder="Wieviel 2-Fr. Stücke?"><span>&nbsp;Stücke</span>
      </div>
      <div class=equal>=</div>
    `
